...
export class RegisterCustomerUseCase {
  ...
  async execute(input: RegisterCustomerInput) {
    ...
    const keyBase = safeFileId(identification); const identificationKey = input.identificationFront ? `clientes/identificaciones/${keyBase}.${safeFileExtension(input.identificationFront)}` : null; const propertyKey = input.propertyPhoto ? `clientes/casas-negocios/${keyBase}.${safeFileExtension(input.propertyPhoto)}` : undefined;
    const createdKeys: string[] = [];
    try {
      if (input.identificationFront && identificationKey) { await this.storage.save(input.identificationFront, identificationKey); createdKeys.push(identificationKey); }
