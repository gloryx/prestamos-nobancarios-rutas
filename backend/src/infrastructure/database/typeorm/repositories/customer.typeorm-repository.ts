...
export class CustomerTypeOrmRepository implements CustomerRepository {
  ...
  private mapCustomer(row: CustomerOrmEntity): Customer { return { id: row.id, identificationType: row.identificationType as Customer['identificationType'], identification: row.identification, firstName: row.firstName, middleName: row.middleName ?? undefined, firstLastName: row.firstLastName, secondLastName: row.secondLastName ?? undefined, gender: row.gender as Customer['gender'], birthDate: row.birthDate, primaryPhone: row.primaryPhone, secondaryPhone: row.secondaryPhone ?? undefined, email: row.email ?? undefined, nationality: row.nationality as Customer['nationality'], otherNationality: row.otherNationality ?? undefined, identificationFrontFileKey: row.identificationFrontFileKey, observations: row.observations ?? undefined, isActive: row.isActive, createdAt: row.createdAt, updatedAt: row.updatedAt }; }
  private mapAddress(row: CustomerAddressOrmEntity): CustomerAddress { return { id: row.id, customerId: row.customerId, districtCode: row.districtCode, exactAddress: row.exactAddress, latitude: row.latitude === null ? undefined : Number(row.latitude), longitude: row.longitude === null ? undefined : Number(row.longitude), propertyPhotoFileKey: row.propertyPhotoFileKey ?? undefined, createdAt: row.createdAt, updatedAt: row.updatedAt }; }
  ...
  async updateWithAddress(id: string, customer: CustomerUpdate, address: Partial<CustomerAddress>): Promise<{ aggregate: CustomerAggregate; oldIdentificationKey?: string; oldPropertyKey?: string }> {
    try {
      return await this.customers.manager.transaction(async (manager) => {
        ...
        const oldIdentificationKey = current.identificationFrontFileKey; const oldPropertyKey = current.address.propertyPhotoFileKey ?? undefined;
